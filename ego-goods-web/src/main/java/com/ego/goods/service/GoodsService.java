...
//查询商品信息
        ResponseEntity<SpuBo> responseEntity = this.goodsClient.queryGoodsById(spuId);
        if (responseEntity==null){
            return null;
        }
        SpuBo spuBo=responseEntity.getBody();
//查询商品详情
...
//查询品牌信息
        Brand brand =
                this.brandClient.queryBrandByBid(spuBo.getBrandId()).getBody();
/**
 * 对于规格属性的处理需要注意以下几点：
 * 1. 所有规格都保存为id和name形式
...
//按照组构造规格参数
    ...
    private void getSpecialSpec(Map<String, String[]> specs, Map<Integer, String> specName,
                                Map<Integer, Object> specValue, Map<Integer, String> specialParamName, Map<Integer,
...
//因为是放在数组里面，所以要先去除两个方括号，然后再以逗号分割成数组
                        Object object = specValue.get(e.getKey());
                        String s=null;
                        if (object==null){
                            s="";
                        }else {
    ...
    private void getAllSpecifications(List<Map<String, Object>> allSpecs, Map<Integer,
            String> specName, Map<Integer, Object> specValue) {
        String k = "k";
        String v = "v";
        String unit = "unit";
    ...
    public void buildStaticHtml(Map<String, Object> modelMap, Long id) {
